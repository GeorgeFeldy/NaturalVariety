...
namespace NaturalVariety.Projectiles.Minions
{
    public class RaptorMinion : ModProjectile
    {
		...
		// This is mandatory if your minion deals contact damage (further related stuff in AI() in the DayMovement region)
        ...
        public override void AI()
        {
			...
			if (Main.dayTime)
            {
				...
				DayGeneralBehavior(owner, out Vector2 vectorToIdlePosition, out float distanceToIdlePosition);
				DaySearchForTargets(owner, out bool foundTarget, out float distanceFromTarget, out Vector2 targetCenter);
				DayMovement(foundTarget, distanceFromTarget, targetCenter, distanceToIdlePosition, vectorToIdlePosition);
				DayVisuals();
		...
		private void DayGeneralBehavior(Player owner, out Vector2 vectorToIdlePosition, out float distanceToIdlePosition)
		{
		...
		private void DaySearchForTargets(Player owner, out bool foundTarget, out float distanceFromTarget, out Vector2 targetCenter)
		{
		...
		private void DayMovement(bool foundTarget, float distanceFromTarget, Vector2 targetCenter, float distanceToIdlePosition, Vector2 vectorToIdlePosition)
		{
		...
		private void NightGeneralBehavior(Player owner, out Vector2 vectorToIdlePosition, out float distanceToIdlePosition)
		{
			Vector2 idlePosition = owner.Center;
			idlePosition.Y -= 48f; // Go up 48 coordinates (three tiles from the center of the player)
