...
namespace NaturalVariety.Mechanics
{
	...
	public class ILPatchContentSamples : ILoadable
    {
        ...
        private static void HookPatchIds(ILContext il)
        {
			...
			//c.Index++;
			//
			//foreach(int id in toInsert)							  // each existing id that will be mapped 
			//{
			//	c.Emit(OpCodes.Ldloc_0);					      
			//	c.Emit(OpCodes.Ldc_I4, id);						  // current id in list 
			//	c.Emit(OpCodes.Ldloc_0);                           
			//	c.Emit(OpCodes.Ldc_I4, IdMap[id]);                // will be mapped to value in dict 
